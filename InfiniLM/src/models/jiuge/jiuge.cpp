...
void inferDeviceBatch(const JiugeMeta &meta, JiugeDeviceResource &rsrc,
    ...
    for (uint32_t layer = 0; layer < nlayer; layer++) {
        ...
        for (uint32_t req = 0; req < nreq; req++) {
            ...
            // qk
            auto rearrange_q_buf = Tensor::buffer(dt_logits, {nkvh, ngroup * seq_len, dh}, rsrc.memory_pool);
            auto q_rearrange = rearrange_q_buf->view({nkvh, ngroup, seq_len, dh});
            ...
            auto attn_val_buf = Tensor::buffer(dt_logits, {nkvh, ngroup * seq_len, dh}, rsrc.memory_pool);
            linear(attn_val_buf, qk_gemm, v_gemm, 1.f, 0.f, nullptr, nullptr);
            // rearrange attn val
            auto attn_val_gemm = attn_val_buf->view({nkvh, ngroup, seq_len, dh});
        ...
        if (rsrc.use_fused_ffn) {
            ...
            fused_ffn(logits_in, logits_in, idev == 0 ? logits_in : nullptr,
                      rsrc.w_ffn_norm[layer], rsrc.w_ffn_gate_up[layer],
                      rsrc.w_ffn_down[layer], meta.epsilon);
        } else {
            // Non-fused path: 4 separate ops
